...
export function savePlace(place: PlaceRecord): PlaceRecord {
  const store = loadStore();
  if (!store.places) store.places = [];

  const now = new Date().toISOString();
  const existingIdx = store.places.findIndex((p) => p.id === place.id || p.slug === place.slug);
  const existing = existingIdx >= 0 ? store.places[existingIdx] : undefined;

  const aliases = new Set<string>(existing?.slugAliases || []);
  if (place.slugAliases) {
    place.slugAliases.forEach((a: string) => aliases.add(a));
  }
  ...
  const updated: PlaceRecord = {
    ...
    updatedAt: now,
    createdAt: place.createdAt || existing?.createdAt || now
  };
